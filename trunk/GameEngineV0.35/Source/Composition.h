...
namespace Framework
{
	...
	public:
		...
		type * GetComponetType(const std::string& name);
		...
		void AddComponent(const std::string& componentName,GameComponent * c);

		///Get the game object's Id
		GOCId GetId(){return ObjectId;}
	private:

		//The map of the component's name to their instance
		//TODO: STD map is large container there is potentially
		//better ways to store components (sorted array, composers, etc);
		typedef std::map<std::string,GameComponent*> ComponentMapType;
		typedef ComponentMapType::iterator MapIt;
		ComponentMapType Map;
	...
	template<typename type>
	type * GameObjectComposition::GetComponetType(const std::string& name)
	{
		return static_cast<type*>( GetComponent(name) );
	}

	#define has(type) GetComponetType<type>( #type )
